...
namespace parallax::mapping {
    enum class TsdfCell : std::uint8_t { Unknown = 0, Surface = 2 };

    struct SpatialTsdfState {
        ...
        [[nodiscard]] bool gridValid() const noexcept {
            ...
            return voxel_size_m > 0.0F && column_count > 0 && row_count > 0 &&
                   slice_count > 0 && cells.size() == expected;
        }
    };
}
