...
LocalOccupancyProducer::LocalOccupancyProducer(
    ...
    camera_ = nvblox::Camera(
        static_cast<float>(p1[0]), static_cast<float>(p1[5]),
    ...
    body_from_rectified_rotation_ =
        multiply(body_from_raw, transpose(calibration.R1()));
...
LocalOccupancyProducer::execution_policy() const noexcept {
...
nvblox::Transform LocalOccupancyProducer::worldFromRectifiedCamera(
    ...
    transform.translation() =
        world_from_body * body_from_rectified_t + world_from_body_t;
    return transform;
}
...
parallax::core::SubmitResult LocalOccupancyProducer::submit(
    ...
    const auto pose =
        products_.latest<parallax::localization::LocalizationPose>(
    ...
    if (last_integrated_ &&
        pose->metadata.observation == *last_integrated_) {
        return parallax::core::SubmitResult::NoWork;
    }

    const auto depth =
        products_.find_observation<parallax::isp::DepthFrame>(
    ...
    mapper_->clearOutsideRadius(
        world_from_camera.translation(), KeepRadiusM);
