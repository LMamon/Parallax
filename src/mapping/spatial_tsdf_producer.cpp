...
namespace parallax::mapping {
    ...
    std::string_view SpatialTsdfProducer::name() const noexcept {
        return "mapping.spatial_tsdf";
    }

    const std::vector<parallax::core::ProductId>&
    SpatialTsdfProducer::inputs() const noexcept { return inputs_; }

    const std::vector<parallax::core::ProductId>&
    SpatialTsdfProducer::outputs() const noexcept { return outputs_; }
    ...
    parallax::core::SubmitResult SpatialTsdfProducer::submit(
        ...
        */
        if (resolver_.demand(parallax::core::ProductId::SpatialTsdf, parallax::core::DemandSource::FoxgloveSubscriber) == 0) {
        ...
        std::shared_ptr<const SpatialTsdfState> published_state = std::move(state);
        products_.publish(parallax::core::make_product(parallax::core::ProductId::SpatialTsdf, metadata, std::move(published_state)));
