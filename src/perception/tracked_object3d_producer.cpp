...
core::SubmitResult TrackedObject3DProducer::submit(core::ExecutionContext& context){
 ...
 Object3D object{}; if(!stereo_.associateTrack(*track->payload,track->metadata,*depth,context,object)) return core::SubmitResult::NoWork;
 auto set=std::make_shared<Object3DSet>(); set->query=track->payload->target_query; set->query_revision=track->payload->target_revision;
 set->objects.push_back(std::move(object)); if(!set->valid()) return core::SubmitResult::Failed;
 auto metadata=track->metadata; metadata.production_timestamp=std::chrono::steady_clock::now();
 products_.publish(core::make_product<Object3DSet>(core::ProductId::TrackedObject3D,metadata,std::move(set)));
 last_track_observation_=track->metadata.observation; return core::SubmitResult::Submitted;
}
}
