...
TEST(DepthTest, RejectsGeometryOutsideUsefulStereoRange) {
    ...
    const std::array<std::int16_t, Width> disparity{20, 10, 2, 1, 0, -1};

    parallax::cuda::CudaBuffer disparity_device, depth_device;
    ASSERT_TRUE(disparity_device.allocate(Width, 1, 1, sizeof(std::int16_t)));
    ASSERT_TRUE(depth_device.allocate(Width, 1, 1, sizeof(float)));
...
TEST(DepthTest, RejectsInvalidRangeConfiguration) {
    ...
    EXPECT_FALSE(parallax::cuda::disparityToDepth(
        ...
        parallax::stereo::MinUsefulDepthM, stream));
    ASSERT_EQ(cudaStreamDestroy(stream), cudaSuccess);
}
